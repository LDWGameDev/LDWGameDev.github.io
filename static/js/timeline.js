...
(function () {
    ...
    function applyBar(bar) {
        ...
        const dur = fmtDuration(startIdx(start), endIdx(end));
        const metaParts = [
            fmtDate(start) + " – " + fmtDate(end),
            dur,
        ];
        if (bar.dataset.location) metaParts.push(bar.dataset.location);
        if (bar.dataset.type) metaParts.push(bar.dataset.type);
    ...
    // Default: show the latest role (largest end index; ties broken by largest start).
